      ...
      {isQuestion ? (
        ...
        <>
          ...
          <motion.div key={"search-box"}  className='search-box' initial={{ scale: 0.7, opacity: 0, boxShadow: "0px 16px 32px rgba(0, 0, 0, 0.048)" }} animate={{ scale: 1, opacity: 1 }} whileHover={{ boxShadow: "0px 16px 32px rgba(0, 0, 0, 0.08)" }}>
            <input key={"search-input"} autoComplete="on"  placeholder='Search anything...' value={searchQuery} onChange={(e) => {setSearchQuery(e.target.value)}} onKeyDown={(e) => {if (e.key === "Enter") {search(e.currentTarget.value)}}}/>
            <motion.button key={"search-submit"}  initial={{ scale: 1.05 }} whileTap={{ scale: 0.85 }} onClick={() => {search(searchQuery)}}>
              <motion.span key={"shimmer"} className='shimmer'/>
              <div key={"btn-txt"} style={{ display: 'flex', flexDirection: "row", gap: 3 }}>
                <IoSearch key={"search-icon"} size={15}/>Search
              </div>
            </motion.button>
          </motion.div>
